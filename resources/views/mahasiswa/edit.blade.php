                    ...
                    <form class="forms-sample" action="{{ route('mahasiswa.update', $mahasiswa->id) }}" method="post"
                        ...
                        <div class="row">
                            <div class="form-group col-lg-6">
                                ...
                                <input type="text" class="form-control" name="npm"
                                    placeholder="Nomor Pokok Mahasiswa" value="{{ $mahasiswa->npm }}" disabled>
                                @error('npm')
                        ...
                        <div class="row">
                            <div class="form-group col-lg-6">
                                ...
                                <input type="text" class="form-control" name="kota_lahir" placeholder="Nama Kota Lahir"
                                    value="{{ $mahasiswa->kota_lahir }}">
                                @error('kota_lahir')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="form-group col-lg-6">
                                ...
                                <input type="date" class="form-control" name="tanggal" placeholder=""
                                    value="{{ DateTime::createFromFormat('l d m y',$mahasiswa->tanggal)->format('Y-m-d')}}">
                                @error('tanggal')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group col-lg-6">
                                ...
                                <select name="prodi_id" class="form-control js-example-basic-single">
                                    ...
                                    @foreach ($prodi as $item)
                                        <option @if ($mahasiswa->prodi_id == $item['id']) selected @endif
                                            value="{{ $item['id'] }}">{{ $item->fakultas->nama_fakultas }} -
